...
class EventListener implements Listener
{
    ...
    public function onInteract(PlayerInteractEvent $event)
    {
        ...
        if (isset($this->plugin->creating[$id = ($player = $event->getPlayer())->getUniqueId()->getBytes()])) {
            if ($event->getAction() === $event::RIGHT_CLICK_BLOCK) {
                ...
                $player->sendMessage(TF::YELLOW . 'Selected position: X' . $x . ', Y: ' . $block->getPosition()->y . ', Z: ' . $z . ', Level: ' . $block->getPosition()->getWorld()->getDisplayName());
                if (!isset($this->plugin->extended[$id = ($player = $event->getPlayer())->getUniqueId()->getBytes()])) {
                    $this->plugin->creating[$id][] = [$x, $block->getPosition()->y, $z, $block->getPosition()->getWorld()->getDisplayName()];
                } else {
                    if (count($this->plugin->creating[$id]) == 0) {
                        $this->plugin->creating[$id][] = [$x, 0, $z, $block->getPosition()->getWorld()->getDisplayName()];
                    } elseif (count($this->plugin->creating[$id]) >= 1) {
                        $this->plugin->creating[$id][] = [$x, 255, $z, $block->getPosition()->getWorld()->getDisplayName()];
                    }
                }
        ...
        if (($reg = $this->plugin->getRegionByPlayer($player)) !== "") {
            if ($reg->getFlag("pluginbypass") === "false") {
                ...
                if ($reg->getFlag("use") === "false") {
                    if (($player->hasPermission("worldguard.usechest." . $reg->getName()) || $player->hasPermission("worldguard.usechest")) && $block === BlockLegacyIds::CHEST)
                        return;
                    if (($player->hasPermission("worldguard.usechestender." . $reg->getName()) || $player->hasPermission("worldguard.usechestender")) && $block === BlockLegacyIds::ENDER_CHEST)
                        return;
                    if (($player->hasPermission("worldguard.usetrappedchest." . $reg->getName()) || $player->hasPermission("worldguard.usetrappedchest")) && $block === BlockLegacyIds::TRAPPED_CHEST)
                        return;
                    if (($player->hasPermission("worldguard.enchantingtable." . $reg->getName()) || $player->hasPermission("worldguard.enchantingtable")) && $block === BlockLegacyIds::ENCHANTING_TABLE)
                        return;
                    if (($player->hasPermission("worldguard.usefurnaces." . $reg->getName()) || $player->hasPermission("worldguard.usefurnaces")) && $block === BlockLegacyIds::BURNING_FURNACE || $block === BlockLegacyIds::FURNACE)
                        return;
                    if (($player->hasPermission("worldguard.usedoors." . $reg->getName()) || $player->hasPermission("worldguard.usedoors")) && ($block === BlockLegacyIds::ACACIA_DOOR_BLOCK || $block === BlockLegacyIds::BIRCH_DOOR_BLOCK || $block === BlockLegacyIds::DARK_OAK_DOOR_BLOCK || $block === BlockLegacyIds::IRON_DOOR_BLOCK || $block === BlockLegacyIds::JUNGLE_DOOR_BLOCK || $block === BlockLegacyIds::OAK_DOOR_BLOCK || $block === BlockLegacyIds::SPRUCE_DOOR_BLOCK || $block === BlockLegacyIds::WOODEN_DOOR_BLOCK))
                        return;
                    if (($player->hasPermission("worldguard.usetrapdoors." . $reg->getName()) || $player->hasPermission("worldguard.usetrapdoors")) && ($block === BlockLegacyIds::IRON_TRAPDOOR || $block === BlockLegacyIds::TRAPDOOR || $block === BlockLegacyIds::WOODEN_TRAPDOOR))
                        return;
                    if (($player->hasPermission("worldguard.usegates." . $reg->getName()) || $player->hasPermission("worldguard.usegates")) && ($block === BlockLegacyIds::ACACIA_FENCE_GATE || $block === BlockLegacyIds::BIRCH_FENCE_GATE || $block === BlockLegacyIds::DARK_OAK_FENCE_GATE || $block === BlockLegacyIds::FENCE_GATE || $block === BlockLegacyIds::JUNGLE_FENCE_GATE || $block === BlockLegacyIds::OAK_FENCE_GATE || $block === BlockLegacyIds::SPRUCE_FENCE_GATE))
                        return;
                    if (($player->hasPermission("worldguard.useanvil." . $reg->getName()) || $player->hasPermission("worldguard.useanvil")) && ($block === BlockLegacyIds::ANVIL))
                        return;
                    if (($player->hasPermission("worldguard.usecauldron." . $reg->getName()) || $player->hasPermission("worldguard.usecauldron")) && ($block === BlockLegacyIds::CAULDRON_BLOCK))
                        return;
                    if (($player->hasPermission("worldguard.usebrewingstand." . $reg->getName()) || $player->hasPermission("worldguard.usebrewingstand")) && ($block === BlockLegacyIds::BREWING_STAND_BLOCK))
                        return;
                    if (($player->hasPermission("worldguard.usebeacon." . $reg->getName()) || $player->hasPermission("worldguard.usebeacon")) && ($block === BlockLegacyIds::BEACON))
                        return;
                    if (($player->hasPermission("worldguard.usecraftingtable." . $reg->getName()) || $player->hasPermission("worldguard.usecraftingtable")) && ($block === BlockLegacyIds::BEACON))
                        return;
                    if (($player->hasPermission("worldguard.usenoteblock." . $reg->getName()) || $player->hasPermission("worldguard.usenoteblock")) && ($block === BlockLegacyIds::NOTE_BLOCK))
                        return;
                    if (($player->hasPermission("worldguard.usepressureplate." . $reg->getName()) || $player->hasPermission("worldguard.usepressureplate")) && ($block === BlockLegacyIds::WOODEN_PRESSURE_PLATE || $block === BlockLegacyIds::LIGHT_WEIGHTED_PRESSURE_PLATE || $block === BlockLegacyIds::HEAVY_WEIGHTED_PRESSURE_PLATE || $block === BlockLegacyIds::STONE_PRESSURE_PLATE))
                        return;
                    if (($player->hasPermission("worldguard.usebutton." . $reg->getName()) || $player->hasPermission("worldguard.usebutton")) && ($block === BlockLegacyIds::STONE_BUTTON || $block === BlockLegacyIds::WOODEN_BUTTON))
                        return;
                ...
                if (!$player->hasPermission("worldguard.edit." . $reg->getName()) && !$player->hasPermission("worldguard.edit")) {
    ...
    {
        $block = $event->getBlock();
        $position = new Position($block->getPosition()->x, $block->getPosition()->y, $block->getPosition()->z, $block->getPosition()->getWorld());
        ...
        if (($region = $this->plugin->getRegionFromPosition($position)) !== "") {
            if ($region->getFlag("pluginbypass") === "false") {
                if ($region->getFlag("block-place") === "false") {
                    if ($event->getPlayer()->hasPermission("worldguard.place." . $region->getName()) || $event->getPlayer()->hasPermission("worldguard.block-place." . $region->getName()) || $event->getPlayer()->hasPermission("worldguard.place") || $event->getPlayer()->hasPermission("worldguard.block-place")) {
                        return true;
    ...
    public function onCommand(PlayerCommandPreprocessEvent $event)
    {
        if ($this->plugin->getRegionByPlayer($event->getPlayer()) !== "")
            if (str_starts_with(strtolower($event->getMessage()), '/f claim')) {
